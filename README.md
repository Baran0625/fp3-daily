...
回答結果を Google スプレッドシートに保存すると、スマホで解いてPCで確認、のように
**どの端末で解いても記録が1か所に集約**されます。設定しなければ `localStorage`（端末内）
のみで動きます。

1. 新しい Google スプレッドシートを作成し、URL の
   `https://docs.google.com/spreadsheets/d/【ID】/edit` の【ID】を控える
2. `gas/SheetSync.gs` を GAS プロジェクトに追加（配信用 `Code.gs` と同じプロジェクトでOK）し、
   `SHEET_ID` に貼る
3. デプロイ → 新しいデプロイ → 種類「**ウェブアプリ**」／実行するユーザー「**自分**」／
   アクセスできるユーザー「**全員**」→ 発行された `/exec` のURLを控える
4. `sync.js` の `SYNC_URL` にそのURLを貼って push

仕組み：書き込みは no-cors の POST（CORS/リダイレクトで失敗しにくい）、読み込みは JSONP。
アプリは起動時にローカルで即描画し、その後シートの記録を取り込んで再描画します。
1日1行で upsert（同じ日を解き直すと上書き）。

> 公開サイトなので `SYNC_URL` はソースから見えます。個人利用なら実害は小さいですが、
> 気になる場合は GAS 側で合言葉トークンを検証する等を追加してください。
